...
# ========== КЭШ ПАРСИНГА ==========
parse_cache = {}  # key: tuple(districts_tuple, rooms_tuple, metros_tuple) -> (data, expiry)

def cache_key(districts, rooms, metros):
    return (tuple(sorted(districts)), tuple(sorted(rooms)), tuple(sorted(metros)))

async def fetch_cian(districts, rooms, metros):
    """Асинхронный парсинг ЦИАН с учётом фильтра по комнатам."""
    key = cache_key(districts, rooms, metros)
    now = time.time()
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=15) as resp:
                if resp.status != 200:
                    logger.error(f"ЦИАН вернул статус {resp.status}")
                    return []
        ...
        for card in cards[:10]:
            try:
                ...
                if room_match:
                    ...
                    # Пробуем найти в характеристиках
                    ...
                    if room_match:
                        ...
                        # Студия?
                        if 'студия' in title.lower() or 'студия' in chars_text.lower():
                            rooms_count = 'студия'

                # Если выбраны конкретные комнаты, проверяем соответствие
                if rooms:
                    # Преобразуем rooms_count к формату наших опций
    ...
    while True:
        try:
            ...
            for user_id, filters_json in users:
                ...
                ads = await fetch_cian(districts, rooms, metros)
                if not ads:
                    continue
                ...
                for ad in new_ads[:3]:
                    ...
                    # По комнатам уже отфильтровано в fetch_cian, но для надёжности проверим
                    room_ok = True
                    if rooms:
                        room_type = None
                        rc = ad['rooms']
                    ...
                    if (not districts and not metros and not rooms) or (district_ok and metro_ok and room_ok):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome = (
        ...
        "⚙️ Чтобы начать, оформите подписку и настройте фильтры (округа, количество комнат, станции метро).\n\n"
...
async def toggle_room(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    room = q.data[2:]  # r_1-комнатная -> 1-комнатная
    selected = context.user_data.get('rooms', [])
    if room in selected:
...
async def toggle_metro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    station = q.data[2:]  # m_Арбатская -> Арбатская
    selected = context.user_data.get('metros', [])
...
async def filters_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Завершение настройки фильтров и сохранение."""
    q = update.callback_query
    await q.answer()
    user_id = q.from_user.id
    ...
    filters = {'city': 'Москва', 'districts': districts, 'rooms': rooms, 'metros': metros}
...
async def test_parse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        users = await Database.get_active_subscribers()
        if not users:
            await update.message.reply_text("❌ Нет активных подписчиков. Сначала выдайте себе подписку через /grant")
            return
        ...
        for user_id, filters_json in users[:3]:
            ...
            rooms = filters.get('rooms', [])
            metros = filters.get('metros', [])
            await update.message.reply_text(f"👤 Пользователь {user_id}: округов {len(districts)}, комнат {len(rooms)}, станций {len(metros)}")
        ...
        await update.message.reply_text("✅ Тест завершён. Проверьте логи на Render для деталей.")
