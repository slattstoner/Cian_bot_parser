#!/usr/bin/env python3
# Версия: 6.2.0 (2026-03-01)
# Изменения:
# - Полностью переработана реферальная система (начисление комиссии на баланс)
# - Добавлена оплата Telegram Stars
# - Изменён процесс ручной оплаты TON (команда /pay <payment_id> <txid>)
# - Исправлены ошибки в админ-панели и навигации
# - Рассылка теперь пачками для предотвращения таймаутов
# - Добавлена таблица balances для хранения реферальных средств
...
PAYMENT_PROVIDER_TOKEN = os.environ.get('PAYMENT_PROVIDER_TOKEN', None)  # Для рублей
GITHUB_TOKEN = os.environ.get('GITHUB_TOKEN', None)  # Для приватного репозитория

# Конфигурация обновлений
GITHUB_REPO = "slattstoner/Cian_bot_parser"
GITHUB_BRANCH = "main"
AUTO_UPDATE_CHECK_INTERVAL = 120  # секунд (2 минуты) для режима отладки
...
class Database:
        ...
        async with cls._pool.acquire() as conn:
            ...
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS balances (
                    user_id BIGINT,
                    currency VARCHAR(10) NOT NULL, -- 'TON', 'STARS', 'RUB'
    ...
    async def cleanup_old_ads(cls, days=30):
        """Удаляет объявления старше указанного числа дней"""
        async with cls._pool.acquire() as conn:
            result = await conn.execute('''
                DELETE FROM ads WHERE created_at < NOW() - $1::interval
            ''', timedelta(days=days))
            logger.info(f"Очистка БД: удалено старых объявлений")
    ...
    @classmethod
    async def add_payment(cls, user_id, amount_ton=0, amount_rub=0, amount_stars=0, plan=None, source='ton_manual'):
        async with cls._pool.acquire() as conn:
    ...
    @classmethod
    async def get_pending_payment(cls, payment_id):
        async with cls._pool.acquire() as conn:
    ...
    @classmethod
    async def confirm_payment(cls, payment_id):
        async with cls._pool.acquire() as conn:
            await conn.execute(
    ...
    # ========== Методы для балансов ==========
    @classmethod
    async def add_to_balance(cls, user_id, currency, amount):
        async with cls._pool.acquire() as conn:
            await conn.execute('''
    ...
    @classmethod
    async def get_balance(cls, user_id, currency):
        async with cls._pool.acquire() as conn:
    ...
    @classmethod
    async def deduct_from_balance(cls, user_id, currency, amount):
        async with cls._pool.acquire() as conn:
            await conn.execute('''
...
async def get_page_html_playwright(url, params=None, use_proxy=True):
    ...
    async with async_playwright() as p:
        ...
        if use_proxy and PROXY_LIST:
            ...
            if proxy:
                # Playwright поддерживает прокси при создании браузера или контекста
                # Можно добавить через launch, но здесь для простоты опустим
                pass
...
def matches_filters(ad, filters):
    ...
    sources = filters.get('sources', ['cian', 'avito'])
...
async def pay_ton(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    payment_id = await Database.add_payment(user_id, amount, 0, 0, plan, source='ton_manual')
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if currency == "RUB":
        ...
        if len(parts) >= 2 and parts[0] == 'rub':
            ...
            await handle_referral_commission(user_id, plan, 'RUB', amount)
        ...
        if len(parts) >= 2 and parts[0] == 'stars':
            ...
            amount = PRICES_STARS[plan]
            await handle_referral_commission(user_id, plan, 'STARS', amount)
...
async def activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Активация подписки после ручной оплаты TON (админ)"""
    ...
    row = await Database.get_pending_payment(payment_id)
    if not row:
        await update.message.reply_text("Платёж не найден или уже подтверждён.")
        return
    ...
    # Начисляем комиссию рефереру
    amount = row['amount_ton']
    await handle_referral_commission(user_id, plan, 'TON', amount)
    
    await update.message.reply_text(f"✅ Подписка для {user_id} активирована.")
    await context.bot.send_message(chat_id=user_id, text="✅ Ваша подписка активирована!")

async def grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выдача подписки администратором (бесплатно)"""
    if update.effective_user.id != ADMIN_ID:
        return
...
async def profile_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Профиль пользователя по ID для админа"""
    if update.effective_user.id != ADMIN_ID:
        return
...
async def broadcast_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == 'bc_yes':
        ...
        # Отправляем пачками по 30 с задержкой 1 секунда между пачками
        batch = []
        for i, (user_id, _, _, _) in enumerate(rows, 1):
            ...
            if i % 30 == 0 or i == len(rows):
                for uid in batch:
                    try:
                        ...
                        success += 1
                        await asyncio.sleep(0.05)  # задержка 50 мс между сообщениями
                    except Exception as e:
                        logger.error(f"Ошибка отправки {uid}: {e}")
                batch = []
                await asyncio.sleep(1)  # пауза между пачками
        
        await q.edit_message_text(f"✅ Рассылка завершена. Успешно: {success} из {len(rows)}")
    else:
...
async def debug_on_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'update_checker' not in context.bot_data or context.bot_data['update_checker'].done():
        context.bot_data['update_checker'] = asyncio.create_task(update_checker_loop(context.application))
    await update.message.reply_text("✅ Режим отладки включён. Проверка обновлений каждые 2 минуты.")
...
async def handle_referral_commission(referred_user_id, plan, currency, amount_paid):
    ...
    # Записываем комиссию в таблицу referrals (историю) и начисляем на баланс
    async with Database._pool.acquire() as conn:
        await conn.execute('''
...
async def admin_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Статистика для админа (callback)"""
    if update.effective_user.id != ADMIN_ID:
        return
...
async def admin_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Список пользователей с пагинацией (callback)"""
    if update.effective_user.id != ADMIN_ID:
        return
    ...
    while True:
        try:
            ...
            if debug_mode:
                ...
                try:
                    result = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd='/opt/bot', capture_output=True, text=True)
                    current_commit = result.stdout.strip()
                    
                    if GITHUB_TOKEN:
                        # Для приватного репозитория используем токен в URL
                        subprocess.run(['git', 'fetch', 'origin', GITHUB_BRANCH], cwd='/opt/bot', capture_output=True)
                        result = subprocess.run(['git', 'rev-parse', f'origin/{GITHUB_BRANCH}'], cwd='/opt/bot', capture_output=True, text=True)
                    else:
                        result = subprocess.run(['git', 'ls-remote', 'origin', GITHUB_BRANCH], cwd='/opt/bot', capture_output=True, text=True)
                    ...
                    if remote_commit and remote_commit != current_commit:
                        ...
                        pull_result = subprocess.run(['git', 'pull', 'origin', GITHUB_BRANCH], cwd='/opt/bot', capture_output=True, text=True)
                        if pull_result.returncode == 0:
                            logger.info("Код обновлён. Проверяем зависимости...")
                            req_change = subprocess.run(['git', 'diff', '--name-only', 'HEAD@{1}', 'HEAD'], cwd='/opt/bot', capture_output=True, text=True)
                            if 'requirements.txt' in req_change.stdout:
                                logger.info("Обновляем зависимости...")
                                subprocess.run([sys.executable, '-m', 'pip', 'install', '-r', 'requirements.txt'], cwd='/opt/bot')
                            
                            logger.info("Перезапуск бота...")
                            subprocess.run(['systemctl', 'restart', 'bot.service'])
                            sys.exit(0)
                        else:
                            logger.error(f"Ошибка git pull: {pull_result.stderr}")
                except Exception as e:
...
async def post_init(app: Application):
    ...
    # Запускаем очистку БД раз в день
...
def main():
    ...
    if PAYMENT_PROVIDER_TOKEN:
        ...
        # Если токена нет, всё равно обрабатываем успешные платежи (на случай звёзд)
        app.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, successful_payment))
