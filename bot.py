#!/usr/bin/env python3
# Версия: 6.0.1 (2026-03-01)
# Исправления:
# - Добавлена колонка deal_type в таблицу ads
# - Исправлена работа кнопок админ-панели (admin_panel_back, admin_panel)
# - Сохранены все функции: парсинг ЦИАН+Авито, рефералы, подписки, модераторы, автообновление
...
class Database:
    ...
    async def init(cls):
        ...
        async with cls._pool.acquire() as conn:
            ...
            # Индекс для быстрого поиска активных подписчиков (обычный, без условий)
    ...
    async def cleanup_old_ads(cls, days=30):
        ...
        async with cls._pool.acquire() as conn:
            ...
            ''', timedelta(days=days))
            logger.info(f"Очистка БД: удалено {result} старых объявлений")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != ADMIN_ID:
        if isinstance(update, CallbackQueryHandler) or (hasattr(update, 'callback_query') and update.callback_query):
            await update.callback_query.answer("⛔ Доступ запрещён.", show_alert=True)
        else:
            await update.message.reply_text("⛔ Доступ запрещён.")
        return
    
    # Определяем, откуда пришёл вызов (сообщение или callback)
    if isinstance(update, CallbackQueryHandler) or (hasattr(update, 'callback_query') and update.callback_query):
...
async def admin_debug_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем текущее состояние из глобальной переменной
...
async def admin_debug_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == 'debug_on':
        ...
        # Запускаем задачу проверки обновлений, если она не запущена
...
async def admin_panel_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != ADMIN_ID:
        return
    # Вызываем admin_panel, передавая update (может быть callback_query)
    await admin_panel(update, context)
        ...
        try:
            ...
            if debug_mode:
                ...
                try:
                    ...
                    # Для приватного репозитория может потребоваться токен
                    if GITHUB_TOKEN:
                        cmd = ['git', 'fetch', 'origin', GITHUB_BRANCH]
                        subprocess.run(cmd, cwd='/opt/bot', capture_output=True)
                    ...
                    if remote_commit and remote_commit != current_commit:
                        ...
                        if pull_result.returncode == 0:
                            ...
                            # Если изменился requirements.txt, устанавливаем новые зависимости
                            ...
                            # Завершаем текущий процесс (он будет перезапущен systemd)
                            sys.exit(0)
                        else:
...
async def post_init(app: Application):
    ...
    # Проверяем, был ли включён режим отладки (можно хранить в БД, но для простоты начинаем с выключенного)
