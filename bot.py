...
METRO_LINES = {
    ...
    'filevskaya': {'name': '🚇 Филёвская линия', 'stations': [
        ...
        "Кунцевская", "Москва-Сити", "Деловой центр", "Шелепиха", "Хорошёво", "Зорге",
        "Панфиловская", "Стрешнево", "Балтийская", "Коптево", "Лихоборы", "Селигерская",
    ...
    'mcd5': {'name': '🚈 МЦД-5 (Пушкино–Домодедово)', 'stations': [
        "Пушкино", "Мамонтовская", "Клязьма", "Тарасовская", "Челюскинская",
        "Строитель", "Мытищи", "Тайнинская", "Перово", "Авиамоторная",
        "Сортировочная", "Электрозаводская", "Митьково", "Рижская",
        "Останкино", "Петровско-Разумовская", "Лихоборы", "Моссельмаш",
        "Грачёвская", "Ховрино", "Левобережная", "Химки", "Молжаниново",
        "Новоподрезково", "Подрезково", "Сходня", "Фирсановка", "Малино",
        "Зеленоград-Крюково", "Алабушево", "Радищево", "Поварово-1",
        "Поваровка", "Берёзки-Дачные", "Подосинки", "Сенеж", "Солнечногорск",
        "Берёзки", "Жихарево", "Поварово-2", "Крюково", "Сходня", "Химки"
    ]}
}
...
# ========== ПАРСИНГ (остаётся без изменений) ==========
...
async def admin_handle_add_mod(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # По умолчанию права: просмотр тикетов и статистики (без финансов)
...
async def mod_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer()
    total, active, pending, total_income, monthly, open_tickets, ads_count = await Database.get_stats()
    # Модератор видит статистику без финансов (по желанию)
    text = (
