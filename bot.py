#!/usr/bin/env python3
# Версия: 6.3.0 (2026-03-01)
# Изменения:
# - Исправлена ошибка в admin_panel_back (теперь корректно работает с callback_query)
# - Улучшена обработка добавления модератора (флаг сбрасывается, добавлены логи)
# - Добавлен маркетинговый текст на страницу /start
# - Улучшено логирование для отладки
# - Добавлены комментарии для разработчика
...
# ПОЛНЫЙ СПИСОК ЛИНИЙ МЕТРО МОСКВЫ (2026) – здесь оставляем как в предыдущей версии, для краткости не повторяю всё, но в реальном коде он должен быть.
# (В целях экономии места я опускаю список станций, но в вашем коде он остаётся без изменений)
# ... (весь список METRO_LINES и ALL_METRO_STATIONS)
# ========== ЛОГИРОВАНИЕ ==========
logging.basicConfig(
...
async def get_page_html_playwright(url, params=None, use_proxy=True):
    ...
    async with async_playwright() as p:
        ...
        # Прокси можно добавить при создании контекста, если нужно
        # if use_proxy and PROXY_LIST:
        #     proxy = await get_random_proxy()
        #     if proxy:
        #         await context.set_extra_http_headers({'Proxy': proxy}) # так не работает, нужен другой подход
...
async def admin_panel_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Возврат в админ-панель (исправленная версия)"""
    if update.effective_user.id != ADMIN_ID:
        return
    context.user_data.pop('awaiting_mod_user_id', None)
    await admin_panel(update, context)
