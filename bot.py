# Версия: 2.0.0 (2026-02-28)
# - Добавлен выбор типа сделки: продажа/аренда
# - Исправлены дубликаты объявлений
# - Защита от бана Telegram (ограничение скорости)
# - Пустые фильтры не отправляют объявления
...
DEAL_TYPES = ['sale', 'rent']  # Продажа, аренда

# Для удобства добавим отображаемые названия
DEAL_TYPE_NAMES = {
    'sale': '🏠 Продажа',
    'rent': '🔑 Аренда'
}
...
class Database:
    ...
    async def init(cls):
        ...
        async with cls._pool.acquire() as conn:
            ...
            ''')
            # Таблица объявлений (добавили поле deal_type)
            await conn.execute('''
    ...
    async def save_ad(cls, ad):
        """Сохраняет объявление в БД. Возвращает True, если добавлено новое."""
        async with cls._pool.acquire() as conn:
            try:
                ...
                # Проверяем, была ли вставка (PostgreSQL возвращает INSERT 0 1 если вставлено)
...
async def fetch_cian_deal_type(deal_type='sale'):
    """Собирает объявления для указанного типа сделки (sale/rent)."""
    params = {
        'deal_type': deal_type,
        'engine_version': '2',
        'offer_type': 'flat',
    ...
    logger.info(f"Собрано {len(results)} объявлений с ЦИАН ({deal_type})")
    return results

async def fetch_cian_all():
    """Собирает объявления по продаже и аренде, объединяет, убирает дубликаты."""
    sale_ads = await fetch_cian_deal_type('sale')
    rent_ads = await fetch_cian_deal_type('rent')
    all_ads = sale_ads + rent_ads
    # Убираем возможные дубликаты по ad_id (на всякий случай)
    seen = set()
    unique_ads = []
    for ad in all_ads:
...
def matches_filters(ad, filters):
    ...
    deal_type = filters.get('deal_type', 'sale')  # по умолчанию продажа

    # Если ни один фильтр не выбран (кроме deal_type, который может быть по умолчанию) - не отправляем
...
# ========== ФОНОВЫЙ СБОРЩИК ==========
# Ограничитель скорости отправки (не более 20 одновременных)
        ...
        try:
            ...
            for ad in new_ads:
                ...
                for user_id, filters_json in subscribers:
                    ...
                    # Пропускаем пользователей без фильтров
                ...
                if tasks:
                    ...
                    # Небольшая задержка между объявлениями, чтобы не превысить лимиты Telegram
...
async def send_ad_to_user(bot, user_id, ad):
    """Отправляет одно объявление пользователю с ограничением скорости."""
    async with telegram_semaphore:
        ...
        try:
            ...
            # Небольшая задержка после отправки, чтобы не превысить лимит
...
async def start_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['deal_type'] = 'sale'  # по умолчанию продажа
    keyboard = [
...
async def admin_tickets_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tickets = await Database.get_open_tickets()
    if not tickets:
        await update.callback_query.edit_message_text("Нет открытых тикетов.")
        return
    text = "🆘 *Открытые тикеты:*\n\n"
    for t in tickets:
        text += f"#{t['id']} от `{t['user_id']}`: {t['message'][:50]}...\n"
...
async def admin_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("Используйте команду /broadcast <текст> для рассылки.")

async def admin_find_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("Используйте команду /find <user_id> для поиска.")
