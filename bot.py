#!/usr/bin/env python3
# Версия: 6.0.0 (2026-03-01)
# - Автоматическое обновление из GitHub (режим отладки)
# - Очистка БД от старых объявлений
# - Оптимизация под 2 ГБ RAM
# - Полный функционал: парсинг ЦИАН+Авито, рефералы, подписки, админка, модераторы
...
async def admin_debug_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем текущее состояние из глобальной переменной (можно хранить в БД, но для простоты в глобальной)
...
async def admin_debug_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == 'debug_on':
        ...
        # Задача останется, но при следующей проверке выйдет по условию
