...
# Для каждого города зададим примерный список станций метро.
# В реальном проекте можно брать из API или дополнять.
METRO_STATIONS = {
    'Москва': [
...
# Для обратной совместимости оставим CITIES (направления по округам/районам)
CITIES = {
...
cursor.execute('''CREATE TABLE IF NOT EXISTS users (
    ...
    filters TEXT,                -- JSON с городом, списком округов, списком станций метро
...
# ---------- НАСТРОЙКА ФИЛЬТРОВ (множественный выбор) ----------
...
async def filter_city_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала предложим выбрать округа
    ...
    keyboard.append([InlineKeyboardButton("⏩ Пропустить округа", callback_data='filter_districts_done')])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"🏘 Выберите **один или несколько округов** в городе {city} (после выбора нажмите Готово):", parse_mode='Markdown', reply_markup=reply_markup)
...
async def toggle_district(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if district in selected:
        ...
        new_text = f"✅ {district}"  # можно обновить кнопку, но проще пересоздать клавиатуру
    else:
        selected.append(district)
    context.user_data['selected_districts'] = selected

    # Перестраиваем клавиатуру с обновлёнными статусами
    ...
    keyboard.append([InlineKeyboardButton("⏩ Пропустить округа", callback_data='filter_districts_done')])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"🏘 Выберите округа в городе {city} (отмеченные ✅ будут добавлены):", parse_mode='Markdown', reply_markup=reply_markup)

async def filter_districts_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    city = context.user_data['filter_city']
    # Переходим к выбору станций метро
    metros = METRO_STATIONS.get(city, [])
    if not metros:
        # Если станций нет, сразу сохраняем фильтры
        await save_filters_and_finish(update, context)
        return

    context.user_data['selected_metros'] = []
    keyboard = []
    # Для удобства разобьём на столбцы? Но для телефона лучше список.
    for m in metros[:30]:  # ограничим количество, чтобы не уйти в long polling
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"🚇 Выберите **одну или несколько станций метро** в городе {city}:", parse_mode='Markdown', reply_markup=reply_markup)
...
async def toggle_metro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([InlineKeyboardButton("⏩ Пропустить метро", callback_data='filter_metros_done')])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"🚇 Выберите станции метро в городе {city}:", parse_mode='Markdown', reply_markup=reply_markup)
...
async def save_filters_and_finish(update, context):
    """Сохраняет выбранные фильтры в БД."""
    query = update.callback_query
...
async def choose_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    keyboard = [
        [InlineKeyboardButton(f"1 месяц – {PRICES['1month']} ₽", callback_data='plan_1month')],
        [InlineKeyboardButton(f"3 месяца – {PRICES['3months']} ₽ (экономия 100 ₽)", callback_data='plan_3months')],
        [InlineKeyboardButton(f"6 месяцев – {PRICES['6months']} ₽ (экономия 300 ₽)", callback_data='plan_6months')],
        [InlineKeyboardButton(f"12 месяцев – {PRICES['12months']} ₽ (экономия 800 ₽)", callback_data='plan_12months')],
...
async def plan_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💳 Кошелёк Telegram (@wallet)", callback_data='pay_wallet')],
...
async def pay_crypto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Сумма: **{amount} ₽** в эквиваленте USDT (TRC20)\n"
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Вы хотите отправить это сообщение **ВСЕМ** пользователям?\n\n{text}", parse_mode='Markdown', reply_markup=reply_markup)
...
def fetch_cian(city, districts, metros):
    """
    Парсинг ЦИАН с использованием библиотеки cianparser.
    Возвращает список объявлений, соответствующих хотя бы одному округу ИЛИ одной станции метро.
    """
    import cianparser
    import logging
    logger = logging.getLogger(__name__)
    ...
    for item in raw_data:
        ...
        # Определяем округ (может быть None)
        detected_district = item.get('district')
        # Приводим к формату сокращений (для Москвы)
        ...
        # Проверяем, подходит ли объявление под фильтры пользователя
        district_ok = False
        metro_ok = False
        ...
        # Если не выбраны округа и не выбрано метро — считаем, что подходит всё
        if (not districts and not metros) or district_ok or metro_ok:
            ...
            ad = {
                ...
                'owner': is_owner,
                'photos': [],  # библиотека не возвращает фото
...
def check_new_ads():
    ...
    for user_id, filters_json in users:
        ...
        for ad in new_ads[:3]:  # не более 3 за раз
