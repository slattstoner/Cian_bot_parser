...
PROXY_URL = os.environ.get('PROXY_URL', None)  # опционально: http://user:pass@ip:port
...
async def daily_by_metro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает свежие объявления по указанным станциям метро (только для админа)."""
    user_id = update.effective_user.id
    logger.info(f"Команда /daily получена от пользователя {user_id}")
    ...
    if not args:
        await update.message.reply_text(
            ...
            "Пример: /daily Арбатская ПлощадьРеволюции\n"
            "Станции можно писать слитно или с пробелами."
        )
        return
