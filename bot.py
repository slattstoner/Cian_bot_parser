...
def get_user(user_id):
    """Возвращает кортеж (filters, subscribed_until, last_ad_id) или None."""
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute('SELECT filters, subscribed_until, last_ad_id FROM users WHERE user_id = %s', (user_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if row:
        return (row['filters'], row['subscribed_until'], row['last_ad_id'])
    return None
...
async def users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT user_id, subscribed_until FROM users ORDER BY user_id LIMIT 20")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    if not rows:
...
def main():
    ...
    logger.info("🚀 Бот успешно запущен и готов к работе (PostgreSQL)")
