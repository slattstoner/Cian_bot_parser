...
def init_db():
    """Создаёт таблицы, если их нет. Добавляет колонку plan в users, если её нет."""
    ...
    ''')
    # Добавляем колонку plan, если её нет (для старых баз)
    try:
...
def get_user(user_id):
    """Возвращает кортеж (filters, subscribed_until, last_ad_id, plan) или None."""
    conn = get_db_connection()
...
def set_user_filters(user_id, filters_dict):
    """Сохраняет фильтры пользователя (JSON)."""
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('''
...
def activate_subscription(user_id, days, plan=None):
    """Активирует подписку на указанное количество дней. Сохраняет план."""
    import time
...
def update_last_ad(user_id, ad_id):
    """Обновляет последнее отправленное объявление."""
    conn = get_db_connection()
    cur = conn.cursor()
...
def get_total_income():
    """Возвращает общий доход от всех подтверждённых платежей."""
    conn = get_db_connection()
    cur = conn.cursor()
...
async def grant_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выдача подписки админом: /grant user_id days plan (plan: 1month, 3months, ...)"""
    if update.effective_user.id != ADMIN_ID:
        return
    try:
        user_id = int(context.args[0])
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает статистику бота и доход."""
    if update.effective_user.id != ADMIN_ID:
        return
    import time
    now = int(time.time())
    ...
    # Общий доход (все подтверждённые платежи)
    cur.execute("SELECT COALESCE(SUM(amount_ton), 0) FROM payments WHERE status = 'confirmed'")
    total_income = cur.fetchone()[0]

    # Расчёт ежемесячного дохода от активных подписок
    # Выбираем всех активных пользователей с планом
    ...
    for (plan,) in active_plans:
        if plan in PRICES_TON and plan in PLAN_DAYS:
            # Стоимость подписки, делённая на количество дней, умноженная на 30 (месяц)
...
def main():
    ...
    logger.info("🚀 Бот успешно запущен и готов к работе (с HTTP-сервером для Render)")
