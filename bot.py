...
DADATA_API_KEY = os.environ.get('DADATA_API_KEY', '')  # ключ для DaData
...
def get_district_by_address(address):
    """
    Определяет административный округ Москвы по адресу через DaData.
    Возвращает сокращение округа (например, 'СЗАО') или None, если не удалось.
    """
    if not DADATA_API_KEY:
        logger.warning("DADATA_API_KEY не задан, геокодирование отключено")
        return None
    ...
    try:
        ...
        r.raise_for_status()
        result = r.json()[0]
        # Ищем округ в поле 'area' с типом 'округ'
        area_type = result.get('area_type')
        area = result.get('area')
        if area_type == "округ" and area:
            # Сопоставляем с сокращением
            return DISTRICT_MAPPING.get(area)
        else:
            # Если не нашли округ, можно попробовать другие поля, но пока вернём None
            return None
...
async def activate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Активировать подписку после получения оплаты (ищет ожидающий платёж)."""
    ...
    try:
        ...
        if row:
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text="✅ Ваша подписка активирована! Теперь выберите город и район для получения объявлений."
            )
            # Отправляем клавиатуру выбора города
            keyboard = []
            for city in CITIES.keys():
                keyboard.append([InlineKeyboardButton(city, callback_data=f'city_{city}')])
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def grant_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выдать подписку без оплаты (прямая активация). Использование: /grant user_id days"""
    if update.effective_user.id != ADMIN_ID:
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text=f"✅ Администратор выдал вам подписку на {days} дней! Теперь выберите город и район для получения объявлений."
        )
        # Отправляем клавиатуру выбора города
        keyboard = []
        for city in CITIES.keys():
...
async def users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать список пользователей (первые 20)."""
    if update.effective_user.id != ADMIN_ID:
        return
...
async def find_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Найти информацию о пользователе по ID. Использование: /find user_id"""
    if update.effective_user.id != ADMIN_ID:
        return
    try:
        user_id = int(context.args[0])
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Разослать сообщение всем пользователям. Использование: /broadcast текст сообщения"""
    if update.effective_user.id != ADMIN_ID:
        return
    if not context.args:
...
async def back_to_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # Имитируем команду /start, но через callback
    keyboard = [
        [InlineKeyboardButton("💳 Подписаться", callback_data='choose_plan')],
        [InlineKeyboardButton("ℹ️ Мой статус", callback_data='my_status')]
    ]
...
def fetch_cian(city, district):
    ...
    Возвращает список объявлений: [{'id': str, 'title': str, 'link': str, 'price': str,
                                     'address': str, 'metro': str, 'floor': str, 'area': str,
                                     'rooms': str, 'owner': bool, 'photos': [url1, url2, url3],
                                     'district_detected': str}]  # определённый округ
    """
    # ВНИМАНИЕ: Это пример. Необходимо заменить URL, параметры и селекторы.
    # Для демонстрации вернём тестовые данные с разными округами.
    import random
    test_ads = [
        {
            'id': '123456',
            ...
            'price': '12 500 000 ₽',
            'address': 'Москва, ул. Маршала Тухачевского, д. 45',
            'metro': 'м. Октябрьское Поле',
            'floor': '5/9',
            'area': '55 м²',
            'rooms': '2',
            'owner': True,
        ...
        {
            'id': '123457',
            'title': 'Продаётся 1-комнатная квартира',
            'link': 'https://cian.ru/sale/flat/123457/',
            'price': '9 800 000 ₽',
            'address': 'Москва, Ленинский проспект, д. 85',
            'metro': 'м. Университет',
            'floor': '3/12',
            'area': '38 м²',
            'rooms': '1',
        ...
        },
        {
            ...
            'link': 'https://cian.ru/sale/flat/123458/',
            'price': '18 200 000 ₽',
            'address': 'Москва, ул. Свободы, д. 12',
            'metro': 'м. Сходненская',
            'floor': '8/14',
            'area': '78 м²',
            'rooms': '3',
    ...
    # Для каждого объявления пытаемся определить округ по адресу
    for ad in test_ads:
        ad_district = get_district_by_address(ad['address'])
        ad['district_detected'] = ad_district
        logger.info(f"Адрес: {ad['address']} -> округ: {ad_district}")
...
def check_new_ads():
    ...
    for user_id, filters_json in users:
        ...
        district = filters.get('district')  # выбранный пользователем округ
        if not city or not district:
            continue
        # Получаем новые объявления (для города Москва, но в реальности нужно учитывать city)
        ...
        # Фильтруем объявления по округу и проверяем, что не отправляли
        new_ads = []
        for ad in ads:
            if ad['id'] == last_ad_id:
                continue
            detected = ad.get('district_detected')
            # Если округ определён и совпадает с выбранным пользователем, добавляем
            if detected and detected == district:
                new_ads.append(ad)
            else:
                logger.debug(f"Объявление {ad['id']} не подходит по округу (найден {detected}, нужен {district})")
        
        # Отправляем все подходящие новые объявления (можно ограничить количество)
        for ad in new_ads[:3]:  # не больше 3 за раз, чтобы не спамить
