...
async def city_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"Выберите район/направление в городе {city}:", reply_markup=reply_mup)
...
def check_new_ads():
    ...
    for user_id, filters_json in users:
        ...
        for ad in new_ads[:1]:  # пока отправляем только первое новое
