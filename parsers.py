...
async def fetch_cian_deal_type(deal_type: str = 'sale') -> List[Ad]:
    ...
    for card in cards[:30]:
        try:
            ...
            floor = '?/?'
            floor_match = re.search(r'(\d+)\s*/\s*(\d+)\s*эт', full_text)
            if not floor_match:
            ...
            owner_tag = card.find('span', string=re.compile('собственник|без посредников', re.I))
            is_owner = bool(owner_tag) or 'собственник' in full_text or 'без посредников' in full_text
            ...
            for img in img_tags:
                ...
                if src.startswith('//'):
                    src = 'https:' + src
                if ('avatar' not in src and not src.endswith('.svg') and
                        'blank' not in src and 'placeholder' not in src and src.startswith('http')):
                    photos.append(src)

            district_detected = None
            if DADATA_API_KEY and AIOHTTP_AVAILABLE:
...
async def fetch_avito_deal_type(deal_type: str = 'sale') -> List[Ad]:
    ...
    for card in cards[:30]:
        try:
            ...
            is_owner = any(w in full_text for w in ['собственник', 'без посредников', 'частное лицо'])
            ...
            for img in img_tags:
                ...
                if src.startswith('//'):
                    src = 'https:' + src
                if 'avatar' not in src and not src.endswith('.svg') and src.startswith('http'):
                    photos.append(src)
...
async def get_district_by_address(address: str) -> Optional[str]:
    if not DADATA_API_KEY or not AIOHTTP_AVAILABLE:
        return None
    ...
    try:
        async with aiohttp.ClientSession() as sess:
            async with sess.post(url, headers=headers, json=[address], timeout=aiohttp.ClientTimeout(total=10)) as resp:
                res = await resp.json()
        result = res[0]
